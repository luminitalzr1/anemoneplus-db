...
export default function App() {
  ...
  const [dbTab, setDbTab] = useState("list");
          ...
          {[
            ...
            <div key={f.key} style={{ ...S.formGroup, ...(f.span===2?{gridColumn:"1/-1"}:{}) }}>
              ...
              {f.type === "select" ? (
                ...
                <input style={S.input} defaultValue={form[f.key]} placeholder={f.placeholder} key={f.key + JSON.stringify(editing)} onBlur={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} />
              )}
            </div>
          ))}
        </div>
          ...
          {[
            ...
            <div key={f.key} style={{ ...S.formGroup, ...(f.span===2?{gridColumn:"1/-1"}:{}) }}>
              ...
              {f.type === "select" ? (
                ...
                <textarea style={{ ...S.input, height:70, resize:"vertical" }} defaultValue={form[f.key]} placeholder={f.placeholder} key={f.key + (editing||"new")} onBlur={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} />
              )}
            </div>
          ))}
        </div>
  ...
  // ── TABLE VIEW ────────────────────────────────────────────────────────────
const TableView = ({ dbTab, setDbTab }) => (
    <>
      <div style={S.toolbar}>
  ...
  return (
    <div style={S.app}>
      ...
      <main style={S.body}>
       {view === "table" && <TableView dbTab={dbTab} setDbTab={setDbTab} />}
